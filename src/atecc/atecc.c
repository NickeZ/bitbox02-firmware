...
static const memory_interface_t* _memory = NULL;
static const random_interface_t* _random = NULL;
...
static ATCA_STATUS _factory_setup(void)
{
    ...
    if (is_config_locked && !is_data_locked) {
        ...
        _memory->read(ATECC_IO_PROTECTION, io_protection_key);
        ...
        _memory->read(ATECC_AUTH, auth_key);
        ...
        _memory->read(ATECC_ENCRYPTION, encryption_key);
        result = atcab_write_zone(
            ATCA_ZONE_DATA, ATECC_SLOT_ENCRYPTION_KEY, 0, 0, encryption_key, 32);
        if (result != ATCA_SUCCESS) {
            return result;
        }
...
int atecc_setup(const memory_interface_t* memory, const random_interface_t* random)
{
    if (memory == NULL || random == NULL) {
        return SC_ERR_IFS;
    }
    _memory = memory;
    _random = random;
...
static ATCA_STATUS _update_kdf_key(void)
{
    ...
    _platform->random(nonce_contribution);
...
int atecc_hmac(const uint8_t* msg, size_t msg_len, uint8_t* mac_out) {
    ...
    int securechip_result = _atecc_kdf(ATECC_SLOT_ROLLKEY, msg, 32, mac_out);
    if (securechip_result != ATCA_SUCCESS) {
        return SC_ERR;
    }
    ...
    for (int i = 0; i < KDF_NUM_ITERATIONS; i++) {
        ...
        securechip_result = _atecc_kdf(ATECC_SLOT_KDF, kdf_in, 32, kdf_out);
