...
typedef struct {
    ...
    int (* kdf)(securechip_slot_t slot, const uint8_t* msg, size_t msg_len, uint8_t* kdf_out);
...
#define ABORT_IF_NULL(fn) do { if(_fns.fn == 0) { Abort("No " #fn " function"); } } while(0)
...
int securechip_kdf(securechip_slot_t slot, const uint8_t* msg, size_t msg_len, uint8_t* mac_out) {
    ABORT_IF_NULL(kdf);
    return _fns.kdf(slot, msg, msg_len, mac_out);
}
