...
const char* flashing_state_str(enum flashing_state state)
{
    ...
    case FLASHING_STATE_DISABLE_UART:
        return "DISABLE_UART";
    case FLASHING_STATE_ENABLE_UART:
        return "ENABLE_UART";
    case FLASHING_STATE_DONE_DONE:
        return "DONE_DONE";
    default:
        return "(invalid)";
    }
    ...
    case FLASHING_STATE_SENT_FIRMWARE:
        if (self->buf_in_len == 1) {
            if (self->buf_in[0] == self->firmware_chksum) {
                ...
                util_log("failure %02X %02X", self->buf_in[0], self->firmware_chksum);
                self->state = FLASHING_STATE_IDLE;
            }
        }
        break;
    case FLASHING_STATE_SEND_CHKSUM_ACK:
        util_log("ACK checksum");
        self->buf_out_small[0] = ACK;
        *buf_out = &self->buf_out_small[0];
        *buf_out_len = 1;
        self->state = FLASHING_STATE_DONE_DONE;
        break;
    case FLASHING_STATE_DONE_DONE:
    default:
        break;
...
void da14531_rst(void)
{
    ...
    if (id != 0xbc11477) {
        util_log("Read invalid idcode: %x", (unsigned int)id);
    } else {
        util_log("Connected to BT chip");
    }

    //// remap address space
    // TODO: Update CSW to half word
    uint32_t w = dap_read_word(0x50000012);
    w >>= 16;
    w &= ~3;
    w <<= 16;
    dap_write_word(0x50000012, w);

    // Deselect issues a reset
    dap_target_deselect();
    dap_disconnect();
}
