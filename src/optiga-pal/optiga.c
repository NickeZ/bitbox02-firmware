...
#define ABORT_IF_NULL(ptr) do { if((ptr)==0) { Abort("Not initalized"); } } while(0)
...
#define _WAIT(return_status, optiga_lib_status) do { \
    ...
    if (OPTIGA_LIB_SUCCESS != optiga_lib_status) { \
        printf("%s failed eventually (code: 0x%04x)\n", __func__, optiga_lib_status); \
...
static bool _write_config(void) {
    ...
    OPTIGA_UTIL_SET_COMMS_PROTECTION_LEVEL(util, OPTIGA_COMMS_NO_PROTECTION);
    res = _optiga_util_write_metadata_sync(util,
            OPTIGA_DATA_OBJECT_ID_HMAC,
            hmac_metadata,
            sizeof(hmac_metadata));
