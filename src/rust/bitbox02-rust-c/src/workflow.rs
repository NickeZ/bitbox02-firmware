    ...
    unsafe {
        ...
        UNLOCK_STATE
            ...
            .write(TaskState::Running(bitbox02_rust::spawn(
                bitbox02_rust::workflow::unlock::unlock(BITBOX02_HAL.get().as_mut().unwrap()),
            )));
    }
}
...
pub unsafe extern "C" fn rust_workflow_spawn_confirm(
    ...
    unsafe {
        ...
        CONFIRM_STATE
            .get()
            .write(TaskState::Running(bitbox02_rust::spawn(confirm::confirm(
                CONFIRM_PARAMS.get().as_ref().unwrap(),
            ))));
    }
}
