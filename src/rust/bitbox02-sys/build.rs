...
pub fn main() -> Result<(), &'static str> {
    ...
    let mut definitions = vec!["-DPB_NO_PACKED_STRUCTS=1", "-DPB_FIELD_16BIT=1"];
    ...
    let source_includes = if env::var("LIB_TYPE").is_err() {
        vec![
            "test/hardware-mocks/src/mock_cipher.c",
            "test/hardware-mocks/src/mock_diskio.c",
            "test/hardware-mocks/src/mock_memory.c",
            "test/hardware-mocks/src/mock_securechip.c",
            "test/hardware-mocks/src/mock_smarteeprom.c",
            "test/hardware-mocks/src/mock_spi_mem.c",
        ]
    } else {
        vec![]
    };
