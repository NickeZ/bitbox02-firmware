...
use bitbox_board_stm32u5_dk as _;
#[cfg(feature = "board-stm32u5-dk")]
...
fn main() -> ! {
    ...
    if nsbootadd0 != BOOT0_ADDR {
        bitbox_debug::rtt_target::rprintln!(
    ...
    bitbox_debug::rtt_target::rprintln!("nsbootadd0 OK, {:#010x}", nsbootadd0);
    ...
    loop {
        ...
        if read > 0 {
            ...
            if written != response_len {
                bitbox_debug::rtt_target::rprintln!("short API response write: {written}");
            }
        }
        unsafe {
            ffi::HAL_Delay(100);
