...
use bitbox_boot_utils::{
    IMAGE_HEADER_INVALID_CODE_SIZE, IMAGE_HEADER_LEN, IMAGE_HEADER_MAGIC_FIRMWARE,
};
use bitbox03_memory::ImmutablePage;
use bitbox03_memory::{FIRMWARE_ADDR, FIRMWARE_MAX_LEN, FLASH_PAGE_SIZE, IMMUTABLE_PAGE_ADDR};
...
impl<B: Backend> BootloaderApi<B> {
    ...
    fn erase_unused_pages(&mut self, firmware_num_chunks: u16) -> Result<(), ()> {
        ...
        let erase_start = if firmware_len.is_multiple_of(FLASH_PAGE_SIZE) {
    ...
    fn current_firmware_monotonic_version(&self) -> Option<u32> {
        Some(self.current_firmware_header()?.monotonic_version.into())
    }
...
mod tests {
    ...
    use super::*;
    use alloc::vec;
    use bitbox_boot_utils::IMAGE_HEADER_MAGIC_BOOT1;
    use bitbox03_memory::{
        FLASH_BASE_NS, FLASH_TOTAL_SIZE, IMAGE_SIGNATURE_COUNT, IMMUTABLE_PAGE_VERSION,
        P256_PUBLIC_KEY_LEN,
    };
    struct MockBackend {
        flash: Vec<u8>,
    ...
    fn install_firmware(backend: &mut MockBackend, code_size: usize, monotonic_version: u8) {
