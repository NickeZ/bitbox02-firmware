...
pub struct BitBox03UiUnlockAnimation {
    lottie: LvLottie,
...
pub struct UnlockAnimationScreen {
    pub screen: LvObj,
    pub handle: BitBox03UiUnlockAnimation,
}

impl BitBox03UiUnlockAnimation {
    pub async fn play<Timer>(self, ui: &mut BitBox03Ui<Timer>) {
        self.lottie.resume();

        let BitBox03UiUnlockAnimation {
            lottie: _lottie,
            shared_state,
        } = self;
...
pub(super) fn build_unlock_animation() -> UnlockAnimationScreen {
    ...
    let shared_state_cb = shared_state.as_ref() as *const RefCell<UnlockAnimationSharedState>;
    ...
    UnlockAnimationScreen {
        screen,
        handle: BitBox03UiUnlockAnimation {
            lottie,
            shared_state,
        },
    }
