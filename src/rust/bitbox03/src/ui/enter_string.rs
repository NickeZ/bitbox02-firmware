...
const KEY_WIDTH_1: LvButtonmatrixCtrl =
    lvgl::ffi::lv_buttonmatrix_ctrl_t::LV_BUTTONMATRIX_CTRL_WIDTH_1;
...
pub(super) fn build_enter_string_screen(
    ...
    unsafe { keyboard.set_textarea(Some(&textarea)) };
    ...
    let textarea_ptr = textarea.as_ptr();
    add_button(
        &input_controls,
        ...
        false,
        move || unsafe {
            lvgl::ffi::lv_textarea_delete_char(textarea_ptr);
        },
    );
    ...
    add_button(
        ...
        params.longtouch,
        move || {
            responder.resolve(Ok(zeroize::Zeroizing::new(snapshot_text(&textarea))));
        },
    );
